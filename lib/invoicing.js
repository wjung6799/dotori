...
// The discount lines for a SIBLING BUNDLE — several of one family's students
// billed on ONE invoice. Same two $100 discounts, same once-per-quarter rule,
// but decided across the whole bundle at once (and against what the family
// already holds this quarter). Call this BEFORE creating the bundle's
// enrollments, passing them in `items` ([{ studentName }]), so they are not
// counted twice. `baseCents` is the bundle's positive tuition total.
export async function bundleDiscountLines({ userId, quarter, items, baseCents = Infinity }) {
  if (!quarter || !items?.length) return [];
  ...
  const [enrollments, assignments, discounted] = await Promise.all([
    ...
    Invoice.find({ userId, quarter, status: { $ne: 'void' }, 'items.kind': 'discount' }).select('items').lean(),
  ]);
  ...
  // Per-student class counts and the distinct-student set, combining what the
  // family already has this quarter with everything in this bundle.
  const counts = new Map();
  const bump = (n) => n && counts.set(n, (counts.get(n) || 0) + 1);
  for (const e of enrollments) bump(e.studentName);
  for (const a of assignments) bump(a.studentName);
  for (const it of items) bump(it.studentName);
  ...
  if (!given.has(MULTI_CLASS_LABEL) && [...counts.values()].some((c) => c >= 2)) {
    offer(MULTI_CLASS_LABEL, 'A student takes more than one class this quarter');
  }
...
export async function createClassInvoice({ user, enrollment, cls, priceOption = null, issuedBy = '', notes = '' }) {
