...
// the card fee are split, so paying monthly costs exactly what paying at once
...
// Plans keep a card on file for the later charges, so they are card payments
// by construction and always carry the card price.
export function installmentSchedule(invoice, installments) {
  ...
  const fee = quoteFor(invoice.subtotalCents, 'card', invoice.onlineFeeCents).adjustmentCents;
...
// When the next installment should be taken, counted from the plan's own due date
