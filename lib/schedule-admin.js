...
// Stop a series without touching anything already booked: flip the schedule
// inactive so it stops appearing as an opening and can't be booked again, but
// leave every existing Booking (past and future) exactly as it is. Standing
// weekly bookings on it are paused right away so the cron doesn't keep filling
// weeks that are no longer offered. Nothing is refunded.
export async function stopSeries({ schedule }) {
  await dbConnect();
  await TutorSchedule.findByIdAndUpdate(schedule._id, { active: false });
  const { modifiedCount: pausedSeries } = await RecurringBooking.updateMany(
    { scheduleId: schedule._id, status: 'active' },
    { $set: { status: 'paused', pausedReason: 'schedule_removed' } },
  );
  const keptBookings = await Booking.countDocuments({
    scheduleId: schedule._id,
    status: 'scheduled',
    startAt: { $gte: new Date() },
  });
  return { keptBookings, pausedSeries };
}
