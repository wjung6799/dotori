...
const tutorScheduleSchema = new mongoose.Schema({
  ...
  // 'session'  → a normal paid slot (consumes a SessionCredit when booked)
  // 'diagnostic' → a free intro assessment slot, bookable without an account or
  //   credit via the public /api/booking/diagnostic route. Kept out of the paid
  //   /schedule grid. Legacy slots with no `kind` are treated as 'session'.
  kind: { type: String, enum: ['session', 'diagnostic'], default: 'session' },
  // Semi-private (a small group shares the slot) or private (one family has it).
  // They are different products at different rates, so a slot has to say which
  // one it is opening. null on legacy rows; lib/sessionTypes.inferSlotType reads
  // those from capacity so nothing needs backfilling.
