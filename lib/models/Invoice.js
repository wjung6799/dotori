...
const invoiceSchema = new mongoose.Schema({
  ...
  // One invoice, charged over up to three months. The family authorizes the card
  // once; the first installment is taken there and then and the rest are charged
  // off-session by the cron. Splitting into separate invoices was the earlier
  // design and made a family chase three bills for one class.
  plan: {
    installments: { type: Number, default: null }, // 2 or 3; null = pay in full
    chargedCount: { type: Number, default: 0 },
    nextChargeAt: { type: Date, default: null },
    // Saved so later installments can be taken without the family present.
    ...
    status: {
      type: String,
      // active  → more installments to take
      // complete→ every installment cleared
      // failed  → a charge failed repeatedly; the office has to step in
      enum: ['active', 'complete', 'failed', null],
      default: null,
    },
  },

  // Every charge made against this invoice, in order. One entry when paid in
  // full, one per installment on a plan.
  payments: {
    type: [
      new mongoose.Schema(
        {
...
// The cron sweeps on these two together.
invoiceSchema.index({ 'plan.status': 1, 'plan.nextChargeAt': 1 });
