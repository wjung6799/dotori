...
const lineItemSchema = new mongoose.Schema(
  {
    ...
    // Which student this line is for (tuition lines on a multi-seat bill). Kept
    // as data so the bill's names never have to be parsed back out of the text.
...
const invoiceSchema = new mongoose.Schema({
  ...
  // When the office last emailed this invoice to the family (the Send button).
  ...
  // A sibling-bundle invoice covers several enrollments at once; enrollmentId
  // stays set to the first for back-compat, and settlement walks this list.
  enrollmentIds: { type: [{ type: mongoose.Schema.Types.ObjectId, ref: 'Enrollment' }], default: [] },
  classId: { type: mongoose.Schema.Types.ObjectId, ref: 'Class', default: null },
  // Set when this bill sells a session pack the office assigned: the linked
  // SessionCredit sits pending (zero bookable) until the invoice settles, and
  // is deleted if the invoice is voided first.
