...
const invoiceSchema = new mongoose.Schema({
  ...
  // ── Instalment plans ───────────────────────────────────────────
  // A family can split a bill over up to three monthly payments. Splitting voids
  // the original and issues one invoice per instalment, all sharing a planId, so
  // each payment is an ordinary invoice with its own due date and its own
  // Stripe charge — no stored cards, no scheduled debits, nothing to fail
  // silently months later.
  planId: { type: String, default: null, index: true },
  installmentNumber: { type: Number, default: null }, // 1-based
