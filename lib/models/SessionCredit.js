...
const sessionCreditSchema = new mongoose.Schema({
  ...
  // A bank-transfer purchase that has been authorized but not yet cleared. The
  // webhook creates the row on payment_intent.processing with zero remaining
  // sessions (so nothing can book against money that has not arrived), then
  // activates it on .succeeded or deletes it on .payment_failed. Without this
  // the family stares at an unchanged balance for 3–5 days.
