...
export async function getGlobal(slug) {
  try {
    ...
    // depth:1 populates upload relationships (hero/core-values images, and the
    // program cards' PDF file uploads) so their .url is available.
    return (await p.findGlobal({ slug, depth: 1 })) || {};
  } catch (err) {
