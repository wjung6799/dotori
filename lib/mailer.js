...
// The owner is copied on every assessment (free diagnostic) lead, whether it
// came in through the message form or a booked slot, so nothing waits on the
// shared inbox. Overridable per environment; empty string disables the copy.
export const ASSESSMENT_CC =
  process.env.ASSESSMENT_CC_EMAIL === undefined
    ? 'wonjung@dotorischool.org'
    : process.env.ASSESSMENT_CC_EMAIL;

// Unified sender used by all templates below. `cc` is optional and dropped
// when it would duplicate the primary recipient.
async function deliver({ to, subject, html, cc }) {
  const ccList = [].concat(cc || []).filter((a) => a && a.toLowerCase() !== String(to).toLowerCase());
  const msg = { from: FROM, to, subject, html, ...(ccList.length ? { cc: ccList } : {}) };
  if (resend) {
  ...
  const money = (c) =>
    (c || 0) < 0
      ? '\u2212$' + (Math.abs(c) / 100).toLocaleString('en-US', {
          minimumFractionDigits: Math.abs(c) % 100 === 0 ? 0 : 2,
