...
// so it carries the line items, the online card fee, the total and a direct link
// into the portal where they settle it.
      ...
      <div style="font-family:sans-serif;max-width:560px;margin:0 auto;color:#333">
        ...
        <table style="border-collapse:collapse;width:100%;margin:16px 0;font-size:14px">
          ...
          ${
            feeCents > 0
              ? `<tr>
                   <td style="padding:8px 12px;color:#9b8b77;border-bottom:1px solid #f0ede8">Online card payment fee</td>
                   <td style="padding:8px 12px;text-align:right;color:#9b8b77;border-bottom:1px solid #f0ede8">${money(feeCents)}</td>
                 </tr>`
              : ''
          }
          <tr>
            <td style="padding:10px 12px;font-weight:bold">Total by card</td>
            <td style="padding:10px 12px;text-align:right;font-weight:bold;font-size:16px">${money(cardTotalCents)}</td>
          </tr>
        </table>
        ...
        <p style="font-size:13px;color:#888">
          Prefer not to pay the online fee? Zelle and checks are always fee-free —
          just reply to this email and we will record it.
        </p>
        <hr style="border:none;border-top:1px solid #eee;margin:24px 0">
        <p style="font-size:12px;color:#aaa">Dotori School · Reply to this email if you have questions.</p>
      </div>
...
// A scheduled installment went through. Sent because a charge the family did not
// initiate should never be something they only discover on a statement.
export async function sendInstallmentCharged({
  to, parentName, invoiceNumber, amountCents, paymentNumber, paymentCount, cardLast4, siteUrl,
}) {
  await deliver({
    to,
    subject: `Payment ${paymentNumber} of ${paymentCount} received — invoice ${invoiceNumber}`,
    html: `
      <div style="font-family:sans-serif;max-width:560px;margin:0 auto;color:#333">
        <h2 style="color:#8b7355">Dotori School</h2>
        <p>Hi ${esc(parentName)},</p>
        <p>
          We have taken <strong>${usd(amountCents)}</strong> — payment
          <strong>${paymentNumber} of ${paymentCount}</strong> on invoice
          <strong>${esc(invoiceNumber)}</strong>${cardLast4 ? ` from your card ending ${esc(cardLast4)}` : ''}.
        </p>
        <p>
          ${
            paymentNumber >= paymentCount
              ? 'That completes this invoice — thank you.'
              : `Payment ${paymentNumber + 1} follows next month. Nothing is needed from you.`
          }
        </p>
...
// A scheduled installment was declined. Says plainly what happens next, because
// silence here reads as "they took it" until the next attempt fails too.
export async function sendInstallmentFailed({
  to, parentName, invoiceNumber, amountCents, paymentNumber, paymentCount, reason, givingUp, siteUrl,
}) {
  await deliver({
    to,
    subject: `Payment ${paymentNumber} of ${paymentCount} did not go through — invoice ${invoiceNumber}`,
    html: `
      <div style="font-family:sans-serif;max-width:560px;margin:0 auto;color:#333">
        <h2 style="color:#8b7355">Dotori School</h2>
        <p>Hi ${esc(parentName)},</p>
        <p>
          We tried to take <strong>${usd(amountCents)}</strong> — payment
          <strong>${paymentNumber} of ${paymentCount}</strong> on invoice
          <strong>${esc(invoiceNumber)}</strong> — and the card was declined.
        </p>
        <p style="color:#a3261a">${esc(reason)}</p>
        <p>
          ${
            givingUp
              ? 'We have stopped trying so your card is not charged repeatedly. Please get in touch and we will sort it out together.'
              : 'We will try once more tomorrow. If the card has changed, reply to this email and we will update it.'
          }
        </p>
        <p><a href="${siteUrl}/dashboard/billing"
             style="display:inline-block;padding:10px 20px;background:#8b7355;color:#fff;text-decoration:none;border-radius:6px">
