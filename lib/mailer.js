...
// A family has been billed for a class seat. This is the only nudge they get
// that money is owed, so it carries the amount, the bank-transfer saving, and a
// direct link into the portal where they settle it.
export async function sendInvoiceIssued({
  ...
  const due = dueAt
    ...
    : null;
  const saves = cardTotalCents - bankTotalCents;

  await deliver({
    to,
    subject: `Invoice ${invoiceNumber} — ${esc(studentName)}'s place in ${esc(className)}`,
    html: `
      <div style="font-family:sans-serif;max-width:560px;margin:0 auto;color:#333">
        <h2 style="color:#8b7355">Dotori School</h2>
        <p>Hi ${esc(parentName)},</p>
        <p>
          <strong>${esc(studentName)}</strong> has a place in
          <strong>${esc(className)}</strong>${schedule ? ` (${esc(schedule)})` : ''}.
          Here is the invoice for their tuition.
        </p>
        <table style="border-collapse:collapse;width:100%;margin:16px 0">
          <tr>
            <td style="padding:8px 12px;background:#f5f0eb;font-weight:bold;width:45%">Invoice</td>
            <td style="padding:8px 12px">${esc(invoiceNumber)}</td>
          </tr>
          <tr>
            <td style="padding:8px 12px;background:#f5f0eb;font-weight:bold">Pay by bank transfer</td>
            <td style="padding:8px 12px"><strong>${money(bankTotalCents)}</strong>${
              saves > 0 ? ` <span style="color:#1e7a40">(saves ${money(saves)})</span>` : ''
            }</td>
          </tr>
          <tr>
            <td style="padding:8px 12px;background:#f5f0eb;font-weight:bold">Pay by card</td>
            <td style="padding:8px 12px">${money(cardTotalCents)}</td>
          </tr>
          ${
            due
              ? `<tr><td style="padding:8px 12px;background:#f5f0eb;font-weight:bold">Due</td>
                 <td style="padding:8px 12px">${esc(due)}</td></tr>`
              : ''
          }
        </table>
        <p>
        ...
        <p style="font-size:13px;color:#888">
          Bank transfers take 3–5 business days to clear; cards go through right away.
          Prefer to pay by Zelle? Just reply and we will record it.
        </p>
        <hr style="border:none;border-top:1px solid #eee;margin:24px 0">
        <p style="font-size:12px;color:#aaa">Dotori School · Reply to this email if you have questions.</p>
      </div>
