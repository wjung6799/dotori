...
import { inferSlotType, isSessionType } from '@/lib/sessionTypes';
...
export async function attemptBooking({
  ...
  // What kind of session this slot opens. Needed both to pick a matching credit
  // and to record on the booking, so it is resolved once, up here.
  const slotType = inferSlotType(schedule);
  // Whether that kind is the tutor's own declaration or only our guess from the
  // seat count. The split is enforced on declared slots; on an unstamped one it
  // would mean refusing a paying family over an assumption nobody made, so
  // those keep taking any credit exactly as they did before kinds existed.
  const slotTypeDeclared = isSessionType(schedule?.sessionType);
  ...
  if (consumeCredit) {
    ...
    // A credit is bought for a KIND of session — semi-private or private — and
    // may only pay for a slot of that kind. A credit with no type predates types
    // and works for either; narrowing it now would void sessions already paid
    // for. Exact-type credits are spent before untyped ones, so the flexible
    // ones stay available for whatever comes next.
    // Best match first, then credits that work with anything. On an undeclared
    // slot a final unfiltered pass accepts the other kind too, so the guess can
    // never be the reason a booking fails.
    const typeFilters = [{ sessionType: slotType }, { sessionType: null }];
    if (!slotTypeDeclared) typeFilters.push({});
      ...
      for (const tutorFilter of [{ tutorId: schedule.tutorId }, { tutorId: null }]) {
        for (const typeFilter of typeFilters) {
          const expiring = await takeOne(
            { ...tutorFilter, ...typeFilter, expiresAt: { $ne: null, $gte: usableOn } },
            { expiresAt: 1 },
          );
          if (expiring) return expiring;
          const evergreen = await takeOne({ ...tutorFilter, ...typeFilter, expiresAt: null });
    ...
    if (consumed.length < need && !allowWithoutCredit) {
      await rollback();
      // Only name a kind when the tutor actually declared one. On an unstamped
      // slot any credit would have counted, so saying "no private sessions" when
      // the family simply has none at all would send them to buy the wrong pack.
      const kind = slotType === 'private' ? 'private' : 'semi-private';
      const noun = slotTypeDeclared ? `${kind} ` : '';
      return {
        ok: false,
        code: 'no_credit',
        error: isPrivate
          ? `Taking the whole slot uses 2 ${noun}sessions and you don’t have enough.`
          : slotTypeDeclared
            ? `You have no ${kind} sessions left. Sessions are bought per kind, so ${kind} credits are what this slot needs.`
