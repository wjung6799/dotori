...
export function packsForTutor(tutor) {
  const rates = (tutor?.rates || []).filter((r) => r && r.sessions > 0 && r.ratePerHour >= 0);
  if (rates.length === 0) return CREDIT_PACKS;
  ...
  return rates
    ...
    .map((r) => {
      ...
      const amountCents = Math.round(r.ratePerHour * HOURS_PER_SESSION * sessions * 100);
      return {
        // Scoped to the tutor so a pack id can never be redeemed against someone
        // else's prices.
        id: `t${sessions}x${Math.round(r.ratePerHour)}`,
        name: sessions === 1 ? 'Single session' : `${sessions}-Session Package`,
        sessions,
        ...
        lines: [
          sessions === 1 ? 'One session' : `${sessions} sessions`,
          `$${r.ratePerHour}/hour`,
        ],
      };
    });
